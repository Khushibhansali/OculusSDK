...
#include <thread>
#include <d2d1.h>
#include "../../../../../Documents/OCULUSsDK/LibOVR/Projects/Windows/VS2017/OVR_CAPI_0_5_0.h"
using namespace std;
...
int PlatformCore::Run()
{
    ...
    Logfile.open("../VS2017/logOutput.csv", std::ios::out | std::ios::app);
    Logfile << "Time" << "," << "Gyro X" << ","<< "Gyro Y" << ","<< "Gyro Z" << "," << "Accel X" << "," << "Accel Y" << "," << "Accel Z"<< endl;
    ovrSession session = pApp->getSession();
    ...
    auto next_time = 0; // regularly updated time point
    while (!Quit)
    {

        auto start_time_microseconds = start_time + next_time;
        const auto durationSinceEpoch = std::chrono::microseconds(start_time_microseconds);
        ...
        long long int milliseconds_remainder = (start_time_microseconds % 1000000) / 100;


        ovrTrackingState ts = ovr_GetTrackingState(session, next_time, ovrTrue);
        ovrVector3f position = ts.HeadPose.ThePose.Position;
    
        accelerometer = ts.RawSensorData.Accelerometer;
        gyroData = ts.RawSensorData.Gyro;
        magData = ts.RawSensorData.Magnetometer;

      
        std::string mili = to_string(milliseconds_remainder);
        while (mili.length() < 4) {
            mili = "0" + mili;
        }

        Logfile << put_time(std::localtime(&time_after_duration), "%H:%M:%S:") << mili << ",";
        Logfile << gyroData.x << "," << gyroData.y << "," << gyroData.z << "," << accelerometer.x << "," << accelerometer.y << "," << accelerometer.z << endl;
        next_time += 1000; // increment absolute time

        /*

        Logfile << ts.HeadPose.AngularAcceleration.x << "," << ts.HeadPose.AngularAcceleration.y << "," << ts.HeadPose.AngularAcceleration.z << "," <<
            ts.HeadPose.AngularVelocity.x << "," << ts.HeadPose.AngularVelocity.y << "," << ts.HeadPose.AngularVelocity.z << ",";
         */
